...
def procurar_campanhas(uf, cidade, loja_nome):
...
def save_in_spreadsheet(campanha, data):
    try:
        ...
        sheet["A1"] = "Empresa", sheet["A2"] = "Atacadão"
        sheet["B1"] = "Campanha", sheet["B2"] = campanha
        sheet["C1"] = "Data", sheet["C2"] = data
        workbook.save = ENCARTE_DIR/"campanhas_atacadão.xlsx"
    except:
...
try:
    ...
    for uf, (cidade, loja_nome) in LOJAS_ESTADOS.items():
        ...
        if nome_loja_encontrada:
            procurar_campanhas(uf, cidade, nome_loja_encontrada)
            time.sleep(1)
